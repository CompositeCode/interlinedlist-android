...
class ListInviteTest {
    ...
    fun `revocation wins over everything`() {
        ...
        assertThat(revoked.statusAt(now).label).isEqualTo("Revoked")
    }
    ...
    fun `inviteUrl prefers the server url and otherwise builds the canonical path`() {
        assertThat(invite(url = "https://example.test/lists/invite/abc").inviteUrl())
            .isEqualTo("https://example.test/lists/invite/abc")
        assertThat(invite().inviteUrl())
            .isEqualTo("https://interlinedlist.com/lists/invite/tok-1")
    }
    ...
    @Test
    fun `invite roles carry the labels the list access UI shows`() {
        assertThat(InviteRole.VIEWER.label).isEqualTo("Read-only")
        assertThat(InviteRole.EDITOR.label).isEqualTo("Edit")
        assertThat(InviteRole.ADMIN.label).isEqualTo("Admin")
    }
