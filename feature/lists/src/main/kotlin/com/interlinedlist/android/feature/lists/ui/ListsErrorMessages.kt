...
/**
 * Invite-specific wording. The invite endpoints reject with a useful `error` string
 * of their own (invalid address, invalid role, unparseable expiry, the subscriber
 * gate), so the server's message is preferred wherever it exists and only the
 * fallbacks are re-worded for the invite context — a 404 here means the invite (or
 * the caller's ownership of the list) is gone, not the list.
 */
fun AppError.toInviteMessage(): String = when (this) {
    is AppError.Network -> "No connection. Check your network and try again."
    is AppError.SubscriptionRequired -> message ?: "Subscribe to invite people to lists."
    is AppError.Forbidden -> message ?: "Only the list owner can manage invites."
    is AppError.NotFound -> message ?: "That invite is no longer available."
    is AppError.Conflict -> message ?: "That person already has access to this list."
