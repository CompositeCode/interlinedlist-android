...
/** Stable test tags for the list share sheet. */
object ShareTestTags {
    const val SHEET = "shareSheet"
    const val LINKS = "shareLinks"
    const val CREATE = "shareCreate"
    const val EMPTY = "shareEmpty"
    const val PROGRESS = "shareProgress"
    const val ERROR = "shareError"
    fun link(token: String) = "shareLink_$token"
    fun copy(token: String) = "shareCopy_$token"
    fun revoke(token: String) = "shareRevoke_$token"
    fun role(role: ShareRole) = "shareRole_${role.apiValue}"
}

/**
 * Hilt-wired share sheet for a list. Shown as a modal bottom sheet over the list
 * detail; [onDismiss] closes it. Reads its `listId` from the nav SavedStateHandle
 * (see [SHARE_LIST_ID_ARG]).
 */
...
fun ShareRoute(
    ...
    ModalBottomSheet(
        ...
        modifier = modifier.testTag(ShareTestTags.SHEET),
    ) {
        ShareSheetContent(
            state = state,
    ...
    Column(
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ShareRole.entries.forEach { role ->
                FilterChip(
                    ...
                    modifier = Modifier.testTag(ShareTestTags.role(role)),
                )
            }
        }
        ...
        if (state.errorMessage != null) {
            ...
            Text(
                ...
                modifier = Modifier.testTag(ShareTestTags.ERROR),
            )
        }
        ...
        when {
            ...
            ) { CircularProgressIndicator(Modifier.testTag(ShareTestTags.PROGRESS)) }

            state.isEmpty -> Text(
                text = "No share links yet.",
                style = MaterialTheme.typography.bodyMedium,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
                modifier = Modifier.testTag(ShareTestTags.EMPTY),
            )
...
private fun ShareLinkRow(
    ...
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .testTag(ShareTestTags.link(link.token)),
    ) {
        Row(
            modifier = Modifier
                .fillMaxWidth()
            ...
            IconButton(
                onClick = onCopy,
                modifier = Modifier.testTag(ShareTestTags.copy(link.token)),
            ) { Icon(Icons.Outlined.Share, contentDescription = "Copy link") }
            IconButton(
                onClick = onRevoke,
                modifier = Modifier.testTag(ShareTestTags.revoke(link.token)),
