...
/**
 * An email invite to a list: a role granted to an *email address* rather than to an
 * account, so the invitee need not have signed up yet and the list stays private
 * throughout. Unlike a [ShareLink] (a bearer capability anyone holding it can use),
 * the [token] only becomes access once someone signed in with that verified address
 * claims it.
 *
...
data class ListInvite(
    ...
    fun inviteUrl(baseUrl: String = ShareLink.INTERLINEDLIST_BASE_URL): String =
        url?.takeIf { it.isNotBlank() } ?: "${baseUrl.trimEnd('/')}/lists/invite/$token"
}
 ...
 * client-side — see [ListInvite.statusAt].
 */
 ...
 * vocabulary (`watcher` / `collaborator` / `manager`) while the UI uses this
 * module's labels (Read-only / Edit / Admin). Unknown values map to [VIEWER] so an
 * invite is never over-privileged.
 */
enum class InviteRole(val apiValue: String, val label: String) {
    VIEWER("watcher", "Read-only"),
    EDITOR("collaborator", "Edit"),
    ADMIN("manager", "Admin");
 ...
 * server does (it stores the address lowercased/trimmed and 400s on an invalid one
 * with `A valid email address is required`). Shared by the UI — to keep Send
 * disabled and show an inline error — and by the repository, which refuses to issue
 * a request for an invalid address.
 */
object InviteEmail {
